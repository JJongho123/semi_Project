<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>

<%@ page import="net.review.db.*"%>
<%
String id = (String) session.getAttribute("id");
ReviewBean board = (ReviewBean) request.getAttribute("reviewdata");
%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>REVIEW 게시판</title>

<script type="text/javascript">
	function modifyboard() {
		modifyform.submit();
	}
</script>
</head>
<body>
	<!-- 게시판 수정 -->
	<form action="ReviewModifyAction.rv" method="post" name="modifyform">
		<input type="hidden" name="REVIEW_NUM"
			value="<%=board.getREVIEW_NUM()%>"> <input type="hidden"
			name="REVIEW_ID" value="<%=id%>">

		<table>
			<tr align="center" valign="middle">
				<td colspan="5">리뷰 게시판<td>
			</tr>

			<tr>
				<td height="16" style="font-family: 돋움; font-size: 12">
					<div align="center">제 목</div>
				</td>
				<td><input name="REVIEW_SUBJECT" size="50" maxlength="100"
					value="<%=board.getREVIEW_SUBJECT()%>"></td>
			</tr>

			<tr>
				<td height="16" style="font-family: 돋움; font-size: 12">
					<div align="center">내 용</div>
				</td>
				<td><textarea rows="15" cols="67" name="REVIEW_CONTENT"><%=board.getREVIEW_CONTENT()%></textarea>
				</td>
			</tr>
			<%
			if (!(board.getREVIEW_IMAGE() == null)) {
			%>
			<tr>
				<td height="16" style="font-family: 돋움; font-size: 12">
					<div align="center">파일 첨부</div>
				</td>
				<td>&nbsp;&nbsp;<%=board.getREVIEW_IMAGE()%>
				</td>
			</tr>
			<%
			}
			%>
			<tr bgcolor="#cccccc">
				<td colspan="2" style="height: 1px;"></td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
			</tr>

			<tr align="center" valign="middle">
				<td colspan="5"><font size="2"> <a
						href="javascript:modifyboard()">[수정]</a>&nbsp;&nbsp; <a
						href="javascript:history.go(-1)">[뒤로]</a>&nbsp;&nbsp;
				</font></td>
			</tr>
		</table>

	</form>
</body>
</html>